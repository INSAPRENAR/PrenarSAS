...
class CalendarioTipo1Serializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Calendario
        fields = ['id','calendar_date', 'expected_date', 'id_pedido','order_code', 'id_producto','name', 'amount', 'machine', 'state', 'observation']
...
class CalendarioTipo2Serializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Calendario
        fields = ['id','calendar_date', 'expected_date', 'id_pedido', 'order_code', 'id_producto','name', 'amount', 'dispatch_time', 'state', 'observation']
    def get_order_code(self, obj):
        return obj.id_pedido.order_code if obj.id_pedido else None
    def get_name(self, obj):
        return obj.id_producto.name if obj.id_producto else None
