...
from api_prenar.serializers.calendarioSerializers import CalendarioListSerializer
# ^ ajusta el import del serializer a tu ruta real
#   (en tu mensaje el nombre era CalendarioListSerializer; deja el path correcto)

class DownloadCronogramaProduccionView(APIView):
    """
    ...
    def get(self, request):
        try:
            ...
            qs = Calendario.objects.filter(filters).order_by("-start_date")
            if not qs.exists():
                return Response(
                    {"message": "No se encontraron calendarios con los filtros especificados."},
            ...
            ws = wb.active
            ws.title = "Cronograma Producción"
            ...
            for cal in calendarios:
                ...
                items = cal.get("production") or []  # lista de dicts (JSONField)
            ...
            filename = "Cronograma_Produccion.xlsx"
            if start_date and end_date:
                filename = f"Cronograma_Produccion_{start_date}_a_{end_date}.xlsx"
            elif start_date:
                filename = f"Cronograma_Produccion_desde_{start_date}.xlsx"
            elif end_date:
                filename = f"Cronograma_Produccion_hasta_{end_date}.xlsx"
